...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div id="main-content" style="padding: 40px; ">
                        ...
                        <tbody>
                            <?php foreach ($hoadon as $hoadon): ?>
                                <tr>
                                    ...
                                    <td><?php echo HoaDonModel::getTenNguoiDung($hoadon->MaNguoiDung); ?></td>
                                    <td style="font-weight: bold;"><?php echo number_format($hoadon->TongTien); ?> đ</td>
                                    <td><?php echo $hoadon->DiaChiGiaoHang; ?></td>
                                    <td><?php echo $hoadon->GhiChu; ?></td>
                                    <td><?php echo $hoadon->HinhThucThanhToan; ?></td>
                                    <td><?php echo date("d/m/Y H:i", strtotime($hoadon->NgayLap)) ?></td>
                                    <?php
                                        $color = ''; 
                                    ...
                                    ?>
                                    <td style="font-weight: bold; color: <?php echo $color; ?>"><?php echo $hoadon->TinhTrang; ?></td>

                                    <td style="color: red;">
                                        <a href="?controller=hoadon&action=index&id=<?= $hoadon->MaHoaDon ?>" class="btn btn-success" data-toggle="modal" data-target="#CTHoaDonModal" data-id="<?= $hoadon->MaHoaDon ?>">Xem</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    ...
                    <?php else: ?>
                        <tr>
                            <br>
                            <td colspan="12">Không có hóa đơn được tìm thấy</td>
                        </tr>
                    <?php endif; ?>
    ...
    <div class="modal fade" id="CTHoaDonModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <form action="?controller=hoadon&action=updateTrangThai" method="POST">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Thông tin chi tiết hóa đơn</h5>
                        <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">×</span>
                        </button>
                    </div>
                    ...
                    <div class="modal-footer">
                        <!-- <input type="text" name="mahoadon" value="HD009"/> -->
                        <input type="submit" class="btn btn-primary" value="Lên Đơn"/>
                    </div>
                </div>
            </form> 
        </div>
    ...
    <script>
        $(document).ready(function(){
            $('#CTHoaDonModal').on('show.bs.modal', function (event) {
                var button = $(event.relatedTarget);
                var maHoaDon = button.data('id');

                $.ajax({
                    url: '?controller=hoadon&action=chiTietHoaDon', 
                    method: 'GET',
                    data: { maHoaDon: maHoaDon },
                    success: function(response) {
                        $('#CTHoaDonModal .modal-body').html(response);
                        $('#CTHoaDonModal input[name="mahoadon"]').val(response.maHoaDon);
                    }
                });
            });
        });
