        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div id="main-content" style="padding: 40px; ">
                    <form method="post" action="?controller=sanpham&action=updateSanPham&id=<?php echo $_GET['id'] ?>" enctype="multipart/form-data">
                        <h1>Chỉnh sửa sản phẩm</h1>
                        <div class="left-section">
                            <label>Tên sản phẩm</label>
                            <input type="text" name="tenSanPham" value="<?= $sanpham->TenSanPham ?>" required/> <br>
                            
                            <label>Hình ảnh</label> <br>
                            <img src="views/assets/img/sanpham/<?= $sanpham->HinhAnhChinh ?>" name="hinhAnhChinh" height="100" width="100"/> <br>
                            <input type="file" name="newHinhAnhChinh"/> <br>

                            <label>Mô tả</label>
                            <textarea name="moTa" required rows="5"><?= $sanpham->MoTa ?></textarea> <br>
                            
                            <label>Giá</label>
                            <input type="number" name="gia" value="<?= $sanpham->Gia ?>" required/> <br>
                            
                            <label>Loại da</label>
                            <input type="text" name="loaiDa" value="<?= $sanpham->LoaiDa ?>" required/> <br>

                            <label>Giới tính</label>
                            <div class="chooseGender">
                                <input type="radio" name="gioiTinh" <?php if($sanpham->GioiTinh == "Nam") echo "checked"; ?> value="Nam"/> Nam
                                <input type="radio" name="gioiTinh" <?php if($sanpham->GioiTinh == "Nữ") echo "checked"; ?> value="Nữ"/> Nữ
                                <input type="radio" name="gioiTinh" <?php if($sanpham->GioiTinh == "Unisex") echo "checked"; ?> value="Unisex"/> Unisex
                            </div>
                            
                            <label>Vấn đề về da</label>
                            <input type="text" name="vanDeVeDa" value="<?= $sanpham->VanDeVeDa ?>" required/> <br>
                        </div>
                        <div class="right-section">
                            <label>Thành phần chính</label>
                            <textarea name="thanhPhanChinh" required rows="5"><?= $sanpham->ThanhPhanChinh ?></textarea> <br>
                            
                            <label>Thành phần chi tiết</label>
                            <textarea name="thanhPhanChiTiet" required rows="5"><?= $sanpham->ThanhPhanChiTiet ?></textarea> <br>
                            
                            <label>Số lượng</label>
                            <input type="number" name="soLuongTon" value="<?= $sanpham->SoLuongTon ?>" required/> <br>

                            <label>Thương hiệu</label>
                            <select name="maThuongHieu" required>
                                <?php foreach ($thuonghieu as $thuonghieu): ?>
                                    <option hidden>Chọn</option>
                                    <option value="<?php echo $thuonghieu->MaThuongHieu; ?>" <?php if ($sanpham->MaThuongHieu == $thuonghieu->MaThuongHieu) echo 'selected'; ?>><?php echo $thuonghieu->TenThuongHieu; ?></option>
                                <?php endforeach; ?>
                            </select> <br>
                            ...
                            <select name="maLoai" required>
                                <?php foreach ($loai as $loai): ?>
                                    <option hidden>Chọn</option>
                                    <option value="<?php echo $loai->MaLoai; ?>" <?php if ($sanpham->MaLoai == $loai->MaLoai) echo 'selected'; ?>><?php echo $loai->TenLoai; ?></option>
                                <?php endforeach; ?>
                            </select> <br>

                            <label><input type="checkbox" <?php if($sanpham->MaKhuyenMai != null) echo "Checked"; ?> name="cbKhuyenMai" onchange="toggleSelect()" /> Khuyến mãi</label> 
                            <select name="maKhuyenMai" required style="display: none;">
                                <?php foreach ($khuyenmai as $khuyenmai): ?>
                                    <option hidden>Chọn</option>
                                    <option value="<?php echo $khuyenmai->MaKhuyenMai; ?>" <?php if ($sanpham->MaKhuyenMai == $khuyenmai->MaKhuyenMai) echo 'selected'; ?>><?php echo $khuyenmai->TenKhuyenMai; ?></option>
                                <?php endforeach; ?>
                            </select> <br>
                        ...
                        <input type="submit" value="Sửa" required/>
                    </form>
                </div>
                <script>
                    //Gọi hàm toggleSelect() khi trang được refresh
